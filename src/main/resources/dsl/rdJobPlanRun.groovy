...
def call(args = [:]) {
	...
	if (showFullLog == null) {
		println("INFO: 'showFullLogs' parameter not provided, hidding full log.")
		showFullLog = false
	}
	...
	try {
		...
		if (!asynchronous) {
			RapidDeployConnectorProxy.checkJobStatus(this, serverUrl, authToken, jobRequestOutput, showIndividualLogs, showFullLog)
		}
	} catch (Exception e) {
		throw e
	}
